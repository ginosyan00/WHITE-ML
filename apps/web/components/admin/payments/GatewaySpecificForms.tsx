    ...
    <div className="space-y-4">
      ...
      <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">
          AMD Account Credentials <span className="text-red-500">*</span>
        </h3>
        <p className="text-xs text-gray-600 mb-4">
          Test Mode ON → օգտագործվում են test credentials<br/>
          Test Mode OFF → օգտագործվում են production credentials
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Username <span className="text-red-500">*</span> <span className="text-xs font-normal text-gray-500">(Merchant ID / Test ID)</span>
            </label>
            <input
              type="text"
              value={config.accounts?.AMD?.username || ''}
              onChange={(e) => updateAMDAccount('username', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent"
              placeholder="ArCa-ից ստացած AMD Merchant ID"
              required
            />
            <p className="mt-1 text-xs text-gray-500">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Password <span className="text-red-500">*</span> <span className="text-xs font-normal text-gray-500">(Secret Key)</span>
            </label>
            <input
              type="password"
              value={config.accounts?.AMD?.password || ''}
              onChange={(e) => updateAMDAccount('password', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent"
              placeholder="ArCa-ից ստացած AMD Secret Key"
              required
            />
            <p className="mt-1 text-xs text-gray-500">
              Test-ի համար → test secret key, Production-ի համար → production secret key
